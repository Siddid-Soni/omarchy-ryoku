...
// Private-use-area sentinels: characters Qt's date formatter will never
// treat as a format specifier and no real clock format will ever contain,
// used to bracket the day-of-month token so its exact position in the
// rendered string can be recovered after formatting -- without guessing at
// column widths or reformatting the date a second time by hand.
var DAY_MARK_OPEN = ""
var DAY_MARK_CLOSE = ""

// Wraps the bare day-of-month token ("d" or "dd") in the sentinels above.
// Must run after weekday substitution (ddd/dddd -> literal text), so a
// surviving lone "d"/"dd" is unambiguously the day number and not a
// fragment of a consumed weekday token. Returns null when the format has no
// such token (pure-time formats like "HH:mm"), so the caller can fall back
// to plain rendering instead of highlighting nothing.
function markDayToken(format) {
  var text = String(format || "")
  var match = /(^|[^d])(d{1,2})($|[^d])/.exec(text)
  if (!match) return null
  var start = match.index + match[1].length
  return text.slice(0, start) + DAY_MARK_OPEN + match[2] + DAY_MARK_CLOSE + text.slice(start + match[2].length)
}

// Inverse of markDayToken, applied to the fully-formatted string: splits it
// into the text before the day number, the day number itself, and the text
// after. Returns null if the sentinels are missing (a format with no day
// token was never marked in the first place).
function splitOnDayMark(renderedText) {
  var text = String(renderedText || "")
  var openIndex = text.indexOf(DAY_MARK_OPEN)
  var closeIndex = text.indexOf(DAY_MARK_CLOSE)
  if (openIndex === -1 || closeIndex === -1 || closeIndex < openIndex) return null
  return {
    before: text.slice(0, openIndex),
    day: text.slice(openIndex + 1, closeIndex),
    after: text.slice(closeIndex + 1)
  }
}
...
// The inverse of dateKey/keyForDate, built from local year/month/day parts
// rather than `new Date("yyyy-MM-dd")` — that string form is parsed as UTC
// midnight by JS, which reads back as the previous day in any negative UTC
// offset once local getters are applied to it.
...
// ---- Calendar cache: everything the panel needs comes pre-shaped from
//      sync-calendars.py (byDay is a precomputed index), so no date math on
//      events happens here — only lookups.

function emptyCalendarCache() {
  return { generatedAt: "", timezone: "", calendars: [], events: [], byDay: {} }
}

// Always returns a valid shape, even for missing, empty, or malformed text,
// so a FileView load failure or a half-written cache can never crash the
// panel — it just reads as "no events yet".
function parseCalendarCache(text) {
  var raw = String(text === undefined || text === null ? "" : text)
  if (raw.trim() === "") return emptyCalendarCache()
  try {
    var parsed = JSON.parse(raw)
    if (!parsed || typeof parsed !== "object") return emptyCalendarCache()
    return {
    ...
    return emptyCalendarCache()
  }
}

// Event records for one day, in the order the helper already sorted them
// (all-day first, then by start time) — the join key is the same "yyyy-MM-dd"
// the month grid already produces per cell.
function eventsForDay(cache, key) {
  if (!cache || !cache.byDay || !cache.events) return []
  var indices = cache.byDay[String(key)]
  if (!indices || !indices.length) return []
  var out = []
  for (var i = 0; i < indices.length; i++) {
    var event = cache.events[indices[i]]
...
function formatEventTime(event) {
  ...
  return Qt.formatTime(start, "h:mm AP")
}

function calendarHasError(cache) {
  if (!cache || !cache.calendars) return false
  for (var i = 0; i < cache.calendars.length; i++)
    if (cache.calendars[i] && cache.calendars[i].ok === false) return true
  return false
}

// "Stale" is relative to how often we asked to refresh, not a fixed
// duration — a 60-minute refresh interval should not be flagged stale after
// 20 minutes just because a 15-minute one would be.
function calendarIsStale(cache, refreshMinutes) {
  if (!cache || !cache.generatedAt) return false
  var generated = new Date(cache.generatedAt).getTime()
  if (isNaN(generated)) return false
  var staleAfterMs = Math.max(1, Number(refreshMinutes) || 15) * 60000 * 3
  return (Date.now() - generated) > staleAfterMs
}

// One line per failing calendar, or a last-synced note when everything is
// fine — meant for a tooltip, not a banner.
function calendarStatusTooltip(cache) {
  if (!cache || !cache.calendars || !cache.calendars.length) return "No calendars configured"
  var lines = []
  for (var i = 0; i < cache.calendars.length; i++) {
    var entry = cache.calendars[i]
    if (entry && entry.ok === false) lines.push((entry.name || "Calendar") + ": " + (entry.error || "sync failed"))
  }
  if (lines.length === 0) return cache.generatedAt ? "Last synced " + cache.generatedAt : ""
  return lines.join("\n")
}
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    markDayToken: markDayToken,
    splitOnDayMark: splitOnDayMark,
    emptyCalendarCache: emptyCalendarCache,
    parseCalendarCache: parseCalendarCache,
    eventsForDay: eventsForDay,
    formatEventTime: formatEventTime,
    calendarHasError: calendarHasError,
    calendarIsStale: calendarIsStale,
    calendarStatusTooltip: calendarStatusTooltip
  }
}
